...
double rho_star = 0.3;
int N = 300;
double TARGET_TEMP = 0.8; // target temperature for integration in canonical ensemble
int M = 5; // number of thermostats in Nosé-Hoover Chain

string prefix = "300_NPT_TEST_0.3";// Name output
string read_prefix = "/Users/luitbald/CODE/MolecularDynamics/MD_Inputs/lattice_05";                  // Read input from
string folderpath = "/Users/luitbald/DATA/"; // Write output to this folder 
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
double tau_NSH = dt*100; // Nose-Hoover characteristic time
...
std::ofstream coolhist(folderpath + prefix + "_coolhist.txt");
...
//define global vectors of Nosé-Hoover Chain
std::vector<double> eta(M);
std::vector<double> p_eta(M);
std::vector<double> weights =  { //weights for Suzuki-Yoshida decomposition
    0.784513610477560,
    0.235573213359357,
    -1.17767998417887,
...
std::vector<double> G(M); // forces on each thermostat
std::vector<double> Q(M); // masses of M thermostats;
...
/* Nosé-Hoover Propagator via Suzuki-Yoshida Decomposition

    p_η[M] = p_η[M] + (Δα / 4) * G[M]
    
    for j =M-1 --> 1
        p_η[j] = p_η[j] * exp(-(Δα / 8) * p_η[j+1] / Q[j+1])
        p_η[j] = p_η[j] + (Δα / 4) * G[j]
        p_η[j] = p_η[j] * exp(-(Δα / 8) * p_η[j+1] / Q[j+1])
    
    p_i = p_i * exp(-(Δα / 2) * p_η[1] / Q[1])    
    η[j] = η[j] + (Δα / 2) * p_η[j] / Q[j]   (for all j from 1 to M)
...
void init_NHC(){
    Q[0] = Nf * TARGET_TEMP*tau_NSH*tau_NSH;
    for (int i = 1; i<M; i++){
        Q[i] = TARGET_TEMP*tau_NSH*tau_NSH;
    }
}
double kinetic_(){
    double KE = 0;
...
//    vel_verlet(100/dt, true, false); // Measure temperature
...
//        double tscale = sqrt((target_temp)/mtemp);
 //       scale_vel(tscale);
  //      vel_verlet(1000/dt, false, false); // relax, no measurement
   //     vel_verlet(100/dt, true, false); // re-measure temperature
     //   mtemp = measured_temp;
      //  measured_temp = 0;
...
//// Read from initial 
    ...
    init_NHC();
    vel_verlet(10/dt, false, true); // relax 20,000 tau
    //vel_verlet(20000/dt, false, true); // collect 20,000 tau
